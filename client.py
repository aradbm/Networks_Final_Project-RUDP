...
def get_dns_ip():  # Get the DNS server IP address from the DHCP server
    ...
    dhcp_discover = Ether(src=client_mac, dst="ff:ff:ff:ff:ff:ff") / IP(src="0.0.0.0", dst="255.255.255.255") / UDP(
        sport=68, dport=67) / BOOTP(chaddr=client_mac, xid=RandInt()) / DHCP(options=[("message-type", "discover"),
                                                                                      ("requested_addr",
                                                                                       "0.0.0.0"),
                                                                                      "end"])
    ...
    while not discover_received and count < 5:
        sendp(dhcp_discover)
        # print(dhcp_request.summary)
        print("DHCP discover sent, waiting for offer...")
        time.sleep(1)
        for packet in sniff(filter="udp and dst port 68 ", iface=conf.iface, timeout=1):
    ...
    print(temp_ip)
    dhcp_request = Ether(src=client_mac, dst="ff:ff:ff:ff:ff:ff") / IP(src="0.0.0.0", dst="255.255.255.255") / UDP(
        sport=68, dport=67) / BOOTP(chaddr=client_mac, xid=RandInt()) / DHCP(options=[("message-type", "request"),
